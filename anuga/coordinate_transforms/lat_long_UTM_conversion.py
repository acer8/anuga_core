...
def LLtoUTM(*latlong, **kwargs):
    """
    converts an array of lat long points (or a single point) to utm coordinates
    """
    input_len = len(latlong)
    msg1 = '''Input must be in the form of: for a single point: \n Lat, Long; (Lat, Long); [Lat, Long]; array([Lat, Long]) \n Or for a list or array of Lat Long points: \n [[Lat1, Long1], ...]; [(Lat1, Long1), ...]; array([[Lat1, Long1], ...]). \n The optional argument ReferenceEllipsoid can be passed as: Lat, Long, ReferenceEllipsoid for a single point or if working with a list or array of Lat Long points, pass it as a keyword argument (e.g. [[Lat1, Long1], ...], ReferenceEllipsoid=23)'''
	...
	zone, x, y = _LLtoUTM(Lat, Long, ReferenceEllipsoid=ReferenceEllipsoid)
        result = (zone, x, y)

    elif input_len == 2:
    	Lat, Long = latlong
        # 1 lat long point in the form of: lat, long
	zone, x, y = _LLtoUTM(Lat, Long, ReferenceEllipsoid=argvalue)
        result = (zone, x, y)
...
def UTMtoLL(*utm, **kwargs):
    """
    converts an array of utm points (or a single point) to lat long coordinates
    """
    input_len = len(utm)
#    print input_len, 'number of inputs'
    msg1 = '''Input must be in the form of: a single point: \n northing, easting zone or a tuple, list or array: (northing, easting, zone); [northing, easting, zone]; array([northing, easting, zone]). \nFor a list or array of utm points: [[northing1, easting1, zone1], ...]; [(northing1, easting1, zone1), ...]; array([[northing1, easting1, zone1], ...]) or a list or array of utm points (e.g. [[northing1, easting1], ...]) with a zone number or list of zone numbers (e.g. [56, ...]). \nOptional arguments (isSouthernHemisphere, ReferenceEllipsoid) can be passed as: northing, easting, zone, isSouthernHemisphere, ReferenceEllipsoid for a single point or if working with a list or array of utm points, pass them as keyword arguments (e.g. [[northing1, easting1], ...], zone, isSouthernHemisphere=True, ReferenceEllipsoid=23)'''
